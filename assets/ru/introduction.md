...
После того, как трейдер добавлен в компанию под ролью `Trader`, ему со стороны `Head of traders` может быть дан доступ к одному или нескольким роботам, а также к одному или нескольким портфелям. Это доступ выдается в виджете `Users`, доступном только пользователям с ролями `headof traders`/`Head view only`.
